...
namespace goose {
    ...
    static vector<AutoCompleteCandidate> SuggestScalarFunctionName(ClientContext &context) {
        ...
        for (const auto &scalar_function: scalar_functions) {
            AutoCompleteCandidate candidate(scalar_function.get().name, SuggestionState::SUGGEST_SCALAR_FUNCTION_NAME,
                                            0);
            suggestions.push_back(std::move(candidate));
        }
    ...
    in_dollar_quotes:
        for (; pos + 2 < qsize; pos++) {
            if (query[pos] == '$' &&
                ...
                memcmp(&query[pos + 1], dollar_quote_tag.GetData(), dollar_quote_tag.GetSize()) == 0) {
                // tags match
