...
namespace goose {
    ...
    void PEGParser::ParseRules(const char *grammar) {
            ...
            switch (parse_state) {
                case PEGParseState::RULE_NAME: {
                    ...
                    if (c == start_pos) {
                        throw InternalException(
                            "Failed to parse grammar - expected an alpha-numeric rule name (pos %d)", c);
                    }
                ...
                case PEGParseState::RULE_SEPARATOR: {
                    if (grammar[c] == '(') {
                        ...
                        rule.parameters.insert(
                            make_pair(string_t(grammar + parameter_start, c - parameter_start),
                                      rule.parameters.size()));
                        if (grammar[c] != ')') {
                            throw InternalException("Failed to parse grammar - expected closing bracket at position %d",
                                                    c);
                        }
                        c++;
                    } else {
                        if (grammar[c] != '<' || grammar[c + 1] != '-') {
                            throw InternalException(
                                "Failed to parse grammar - expected a rule definition (<-) (pos %d)", c);
                        }
                        c += 2;
                ...
                case PEGParseState::RULE_DEFINITION: {
                    ...
                    if (grammar[c] == '\'') {
                        ...
                        if (grammar[c] == 'i') {
                            throw InternalException(
                                "Failed to parse grammar - unexpected \"i\" found in grammar near rule %s",
                                rule_name.GetString());
                        }
                        ...
                        if (grammar[c] == '(') {
                            ...
                            if (bracket_count == 0) {
                                throw InternalException(
                                    "Failed to parse grammar - unclosed bracket at position %d in rule %s",
                        ...
                        throw InternalException("Unrecognized rule contents in rule %s (character %s)",
                                                rule_name.GetString(),
                                                string(1, grammar[c]));
                    }
                }
        ...
        if (parse_state == PEGParseState::RULE_SEPARATOR) {
            throw InternalException("Failed to parse grammar - rule %s does not have a definition",
                                    rule_name.GetString());
        }
