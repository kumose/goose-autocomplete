...
namespace goose {
        ...
        if (opt_sequence_options.HasResult()) {
            ...
            for (auto seq_option: repeat_seq_options.children) {
                ...
                if (sequence_options.find(seq_result.first) != sequence_options.end()) {
                    throw ParserException("CREATE SEQUENCE option %s has already been specified before.",
                                          seq_result.first);
                }
    ...
    PEGTransformerFactory::TransformSequenceOption(PEGTransformer &transformer,
                                                   optional_ptr<ParseResult> parse_result) {
        auto &list_pr = parse_result->Cast<ListParseResult>();
    ...
    pair<string, unique_ptr<SequenceOption> >
    PEGTransformerFactory::TransformSeqSetIncrement(PEGTransformer &transformer,
                                                    optional_ptr<ParseResult> parse_result) {
        auto &list_pr = parse_result->Cast<ListParseResult>();
